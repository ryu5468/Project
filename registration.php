...
if (isset($_POST['register']))
  {
	...
	// Check desired username for one already existing
	$check = mysql_query("SELECT userName FROM users WHERE userName = '$newUserName'");
	$numRows = mysql_num_rows($check);
	$row = mysql_fetch_row($check);
	$data = $row[0];
	
	// Notify username already exists and password too short
	...
	// Notify username already exists and passwords don't match
